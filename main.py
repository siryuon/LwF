...
with open(args.outfile, 'w') as file:
        ...
        for s in range(0, num_iters, num_classes):
                ...
                train_set = cifar100(root='./data', train=True, classes=all_classes[s:s+num_classes], download=True, transform=transform, mean_image=mean_image)
                train_loader = torch.utils.data.DataLoader(train_set, batch_size=args.batch_size, huffle=True, num_workers=12)

                test_set = cifar100(root='./data', train=False, classes=all_classes[:s+num_classes], download=True, transform=None, mean_image=mean_image)
                test_loader = torch.utils.data.DataLoader(test_set, batch_size=args.batch_size, huffle=False, num_workers=12)
                ...
                for i in range(model.n_known):
                        test_set = cifar100(root='./data', train=False, classes=all_classes[i*num_classes: (i+1)*num_classes], download=True, transform=None, mean_image=mean_image)
                        test_loader = torch.utils.data.DataLoader(test_set, batch_size=min(500, len(test_set)), shuffle=False, num_workers=12)
